...
def main(item, detectorYolov3, STYLE_IMAGE=None, MASK_IMAGE=None, PATCH_IMAGE=None, ADV_PATH=None, DET_PATH=None):
    style_img_path = './yolov3/tie2.jpg' if STYLE_IMAGE is None else STYLE_IMAGE
    # content_img_path = f'yolov3/data/test/{item}'
    content_img_path = item  # K 传入图片的绝对路径
    image_name = os.path.basename(content_img_path)[:-4]
    mask_path = f'yolov3/outputs/{image_name}/mask.jpg' if MASK_IMAGE is None else MASK_IMAGE  # 获取mask
    patch_path = f'{args.output_dir}/patch/{image_name}.jpg' if PATCH_IMAGE is None else PATCH_IMAGE  # 最终补丁存储路径
    save_path = f'{args.output_dir}/adv_img/{image_name}.jpg' if ADV_PATH is None else ADV_PATH  # 最终对抗样本存储路径
    save_detpath = f'{args.output_dir}/det_img/{image_name}.jpg' if DET_PATH is None else DET_PATH  # 对抗样本检测效果存储路径
...
# K 想把上面main里面的图片检测功能单独拎出来，但是好像有点问题
def detect(item, detectorYolov3):
    content_img_path = item
    image_name = os.path.basename(content_img_path)[:-4]
    mask_path = f'yolov3/outputs/{image_name}/mask.jpg'  # 获取mask
    save_detpath = f'{args.output_dir}/detect/{image_name}.jpg'  # 对抗样本检测效果存储路径
    ...
    det_adv_image = detector.plot(adv_image, names, det, 0.5)  # 检测对抗样本
    save_image(det_adv_image, save_detpath)  # 存储检测结果
    return os.path.dirname(os.path.abspath(save_detpath))


if __name__ == "__main__":
    detectorYolov3 = DetectorYolov3(show_detail=False, tiny=True)
    FROM = 1  # K 从这里main直接运行的话改成0, 从Gradio调用的话改成1
    if FROM == 0:
        ...
        for image in images:
            ...
            main(
                ...
                PATCH_IMAGE=PATCH_IMAGE.format(image_name),
